    ...
    fn calc_attacked_squares(position: &Position, mut piece_pos: u64, _player: &PlayerColour, enemy_king_pos: u64, king_attack_analyzer: &mut KingAttackRayAnalyzer) -> u64 {
        ...
        while piece_pos > 0 {
            ...
            if RANKS[sq_ind] & enemy_king_pos > 0 {
                ...
                king_attack_analyzer.analyze_king_attack_ray(position, sq_ind, ATTACK_RAYS[(sq_ind << 6) + king_sq], true, enemy_king_pos);
            }
...
mod tests {
    ...
    fn test_calc_rook_movements() {
        // 1. Starting position
        let (_, mut position, mut move_list, _) = LegalMovesHelper::init_test_position_from_fen_str(Some("8/8/3K4/8/8/8/Pk1r4/R2N4 w - - 1 2"));
        LegalMovesHelper::check_attack_and_movement_squares(
            Rook::calc_movements(&position, position.wr, &mut move_list, None),
            vec!["b1", "c1", "d1", "a2"],
            vec!["b1", "c1"]
        );
        ...
        LegalMovesHelper::check_attack_and_movement_squares(
            Rook::calc_movements(&position, position.br, &mut move_list, None),
        ...
        let (_, mut position, mut move_list, _) = LegalMovesHelper::init_test_position_from_fen_str(Some("r2q1rk1/pp2ppbp/2p2np1/2pPP1B1/8/Q5np/P1P2pP1/3RKB1R w KQkq - 1 2"));
        LegalMovesHelper::check_attack_and_movement_squares(
            Rook::calc_movements(&position, position.wr, &mut move_list, None),
